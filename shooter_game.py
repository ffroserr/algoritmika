...
font.init()
font1 = font.SysFont("Arial", 30) #SysFont("свой шрифт", размер)
...
for i in range(4):
    asteroid = Asteroid("asteroid.png", randint(0, 630), -5, 65, 65, randint(0, 1))
    asteroids.add(asteroid)

for i in range(1):
    enemy = Enemy("ufo.png", randint(0, 630), -10, 65, 65, randint(1,2)) #Меняется только Х( будет рандомным ) randint(x1,x2)
    monsters.add(enemy)
...
while game:
    ...
    if finish != True:
        ...
        for m in sprites_list:
            enemy = Enemy("ufo.png", randint(0, 630), -5, 65, 65, randint(1,3)) #Меняется только Х( будет рандомным ) randint(x1,x2)
            monsters.add(enemy)
